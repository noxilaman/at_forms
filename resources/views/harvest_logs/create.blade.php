    ...
    <div class="row">
        ...
        <div class="col-xl-12 col-md-12 mb-12">
            <div class="card border-left-primary shadow h-100 py-2">
                <div class="card-body">
                    <form action="{{ route('harvest_logs.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                                    ...
                                    <select class="form-control" id="harvester_id" name="harvester_id">
                                        @foreach ($harvesterList as $harvesterid => $harvester)
                                            <option value="{{ $harvesterid }}"
                                                {{ old('harvester_id') == $harvesterid ? 'selected' : '' }}>
                                                {{ $harvester }}</option>
                                        @endforeach
                                    </select>
                                    @error('harvester_id')
                                    ...
                                    <select class="form-control" id="crop_id" name="crop_id">
                                        @foreach ($cropList as $cropid => $crop)
                                            <option value="{{ $cropid }}"
                                                ...
                                                {{ $activeCrops->crop_id == $cropid ? 'selected' : '' }}>
                                                {{ $crop }}</option>
                                        @endforeach
                                    </select>
                                    @error('crop_id')
                                    ...
                                    <select class="form-control" id="driver_id" name="driver_id">
                                        @foreach ($driverList as $driverid => $driver)
                                            <option value="{{ $driverid }}"
                                                {{ old('driver_id') == $driverid ? 'selected' : '' }}>
                                                {{ $driver }}</option>
                                        @endforeach
                                    </select>
                                    @error('driver_id')
