...
@section('content')
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">{{ __('Question Edit') }} # {{ $question->id }}</h1>
    </div>
                    ...
                    <form action="{{ route('questions.update', $question->id) }}" method="POST">
                        ...
                        <div class="form-group
                            ...
                            <select class="form-control" id="status" name="status">
                                <option value="active" {{ old('status', $question->status) == 'active' ? 'selected' : '' }}>{{ __('Active') }}</option>
                                <option value="inactive" {{ old('status', $question->status) == 'inactive' ? 'selected' : '' }}>{{ __('Inactive') }}</option>
                            </select>
                            @error('status')
                                <span class="help-block
                                    @error('status')
